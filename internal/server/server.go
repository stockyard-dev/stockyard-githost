package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-githost/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-githost"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/repos",s.handleListRepos)
    s.mux.HandleFunc("POST /api/repos",s.handleCreateRepo)
    s.mux.HandleFunc("DELETE /api/repos/{id}",s.handleDeleteRepo)
    s.mux.HandleFunc("GET /api/repos/{id}/issues",s.handleListIssues)
    s.mux.HandleFunc("POST /api/repos/{id}/issues",s.handleCreateIssue)
    s.mux.HandleFunc("POST /api/issues/{id}/close",s.handleCloseIssue)
